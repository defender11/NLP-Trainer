...
`focus-trainer.html` в корне является эталонным файлом и доступен только для чтения.

Проект содержит Webpack-конфигурацию для сборки и локального сервера разработки.
